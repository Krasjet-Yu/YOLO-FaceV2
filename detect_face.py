...
from utils.plots import plot_one_box, plot_text_label
...
if __name__ == '__main__':
    ...
    parser.add_argument('--plot-label', action='store_true', help='show results')
    opt = parser.parse_args()
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = load_model(opt.weights, device)
    detect(model, opt.source, device, opt.project, opt.name, opt.exist_ok, opt.save_img, opt.view_img, opt.plot_label)
