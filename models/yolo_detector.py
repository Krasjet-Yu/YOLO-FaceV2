...
class YOLOV5TorchObjectDetector(nn.Module):
    ...
    def forward(self, img, visualize=False):
        prediction, x = self.model(img, augment=False, visualize=visualize)
        logits = []
        for i in range(len(x)):
            bs, _, _, _, no = x[i].shape  # x(bs,255,20,20) to x(bs,3,20,20,85)

            logits.append(x[i][..., 5:].view(bs, -1, no - 5))
        # logits = torch.tensor([item.cpu().detach().numpy() for item in logits]).to(device=self.device)
